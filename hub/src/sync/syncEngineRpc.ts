/**
 * RPC-domain sub-facade of SyncEngine.
 *
 * `archiveSession` is the only method that has a non-trivial side effect outside the
 * RpcGateway — it must mark the session ended after killing the agent. We inject a
...
export type SessionEndCallback = (payload: { sid: string; time: number }) => void

export class SyncEngineRpc {
    constructor(
        private readonly rpcGateway: RpcGateway,
        private readonly onSessionArchived: SessionEndCallback
    ) {}
    ...
    async archiveSession(sessionId: string): Promise<void> {
        ...
        this.onSessionArchived({ sid: sessionId, time: Date.now() })
    }
