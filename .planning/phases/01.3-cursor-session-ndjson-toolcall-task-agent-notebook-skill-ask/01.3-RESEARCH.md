...
1. **Exact NDJSON keys for Notebook and AskUserQuestion** — **RESOLVED by Plan 01.3-01**
   - Resolution: Human capture checkpoint in 01.3-01 locks native keys from redacted fixtures before Plan 01.3-02 allowlist extension. Pre-capture: use `notebookReadToolCall`/`notebookEditToolCall` and `askUserQuestionToolCall` as placeholders; adjust to capture evidence in 01.3-01-SUMMARY.

2. **Task vs Agent arg discrimination on legacy infer** — **RESOLVED by Plan 01.3-03**
   - Resolution: `inferToolNameFromPayload` uses `description`/`name`/`team_name` → `Task`; `prompt`+`subagent_type` without Task fields → `Agent`. Covered in 01.3-03 unit tests.

3. **Wire parent/child linkage for Trace (D-14)** — **RESOLVED by Plan 01.3-04 (defer)**
   - Resolution: 01.3-04 Task 1 audits wire for `parentCallId`/`parent_call_id`; if absent (expected), SUMMARY records "Trace nesting deferred — wire lacks parent/child callId (D-14)." No nested Trace implementation this phase.
