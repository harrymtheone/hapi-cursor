...
- **D-09:** Web session-scoped projection map; SSE merges patches (`message-received` alone insufficient).
...
1. **Offline migration for schema 12**
   - What we know: Project policy rejects runtime migrations; v9→v10 script exists under `hub/scripts/` as precedent.
   - What's unclear: Whether to ship `migrate-tool-calls-v12.ts` in this phase or document rebuild-only.
   - Recommendation: Planner adds Wave 0 task—either script or explicit “delete/rebuild hub.db” step in PLAN verification.

2. **SSE batching vs per-callId events**
   - What we know: D-08 allows batch; discretion open.
   - Recommendation: Per-callId events (simpler handler, matches single upsert); batch only if ingest batches multiple tools in one tick.
