...
**Phases:** 1–12
**Total Plans:** 60
**Git range:** `a61af67` (2026-05-20) → `8e60ab1` (2026-05-23)
**Net diff:** 917 files changed, +60,520 / −72,559 (≈ −12k LOC net slim-down)

## Overview

Milestone 1 was a pure refactor + cleanup pass on a brownfield fork of upstream HAPI. The journey: first **shrink** by deleting four non-Cursor agent runtimes (Claude, Codex, Gemini+ACP, OpenCode) and three external integration surfaces (Telegram bot, ElevenLabs voice, ServerChan push); then **collapse** multi-user namespacing and built-in tunnel infrastructure that single-user Tailscale usage doesn't need; then **rebuild** the abstractions that Milestone 2 (Cursor incremental features) will depend on — a populated flavor capability table, a shared agent runtime kit, a single wire-contract source in `shared/`, a decomposed hub sync layer, decomposed web components, immutable config, and a focused test suite. The milestone closed with documentation cleanup, push-gated CI (`.github/workflows/verify.yml`), and end-to-end verification on a real Tailscale + phone session.

## Milestone Summary

**Key Decisions (extract):**

- Fork over upstream contribution — Cursor-only direction conflicts with upstream multi-agent goal
- Big deletions before refactors so every downstream change touches a smaller surface
- Flavor capability abstraction (REFA-01) folded into Phase 5 alongside CUT-05
- Documentation cleanup (CUT-12) folded into final verification phase (VRFY-03 ripgrep depends on clean docs)
- SQLite schema mismatch ⇒ refuse start + point to offline migration tool (no runtime migration path)
- Single coordinated wave for `loadConfig()` HIGH-risk cutover (CLI 30+ consumers / Hub 7 consumers)
- SSE event contract becomes strict (`SessionPatchSchema` / `MachinePatchSchema` in `shared/`); heuristic `hasUnknownSessionPatchKeys()` deleted

**Issues Resolved:**

- 3 madge cycles in `cli/src/cursor/` broken (modes.ts leaf extraction)
- 11-file ToolCard cycle broken (`_all.tsx ↔ _results.tsx`)
- pre-existing messageWindow* madge cycles fixed via trim-helper extraction + persistence registrar inversion
- 0 circular dependencies across `cli/` / `hub/` / `web/` (intra-package and cross-package) — `madge:check` enforces on push + PR
- Phase 2 verification-closure plan (02-06) fixed 3 HIGH-severity findings (fetchVoiceToken, /api/bind auth bypass, web/src/lib/languages.ts)

**Issues Deferred (carry-forward backlog M2-BL-01..10):**

- See `.planning/milestones/v1.0-phases/12-docs-cleanup-milestone-verification/12-04-SUMMARY.md` for the full Milestone 2 outstanding backlog list

**Technical Debt Incurred:**

- `reducerTimeline.ts` (925 lines) decomposition deferred — not on Milestone 2 critical path
- Cursor permission-mode mapping helper not yet promoted to `shared/` (cli/hub-side dedup deferred)
- Lint not enforced in CI (`bun run lint` exists but is not wired into verify.yml) — surfaced explicitly in 12-VERIFICATION.md
