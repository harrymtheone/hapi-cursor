...
- [x] **REFH-02**: `SyncEngine`（854 行）拆分；SSE 不再反向依赖 SyncEngine 具体类型，只依赖 `shared/` 的事件类型
- [x] **REFH-03**: Hub 路由模板抽象。加 `parseJsonBody(schema) / withEngine / withSession / withActiveSession / withMachine` helper + 统一 `ApiRouteError`；按职责拆 `hub/src/web/routes/sessions.ts`（lifecycle / config / upload / read-only）
- [x] **REFH-04**: 集中 keepalive 调度器。SSE / SyncEngine / terminalRegistry / notificationHub 的 `setInterval` 统一到一个调度器；`process.exit` 时全部 timer 都被清理（含 SIGINT 测试）

### Web 内部解耦（REFACTOR / WEB）

- [x] **REFW-01**: 打破 ToolCard 11 文件循环依赖（`_all.tsx ↔ _results.tsx`）；加「所有已知 tool 能解析到 renderer」的集成测试
- [x] **REFW-02**: 拆分超大文件：`SessionList.tsx`（990 行）、`message-window-store.ts`（1087 行）、`reducerTimeline.ts`（925 行）、`routes/settings/index.tsx`（847 行）、`AssistantChat/HappyComposer.tsx`（870 行）
- [x] **REFW-03**: 提取重复工具函数到 `shared/`（Levenshtein 距离、base64 大小估算、Cursor permission mode 映射、API query hook 形状）

### 清理与配置（REFACTOR / CONFIG）

- [x] **REFC-01**: 清理 backward-compat 残留。删除 `serverUrl → apiUrl` / `webapp* → publicUrl` 字段别名、`hapi server` 命令别名、所有 SQLite 运行时迁移；SQLite schema 版本不匹配 ⇒ 拒绝启动 + 提示离线迁移工具
- [x] **REFC-02**: mutable config singleton 改造。CLI 与 Hub 改为 `loadConfig()` 返回冻结对象；移除 `_setApiUrl() / _setCliApiToken() / _setExtraHeaders()` 等 setter；通过 dependency injection 注入

### 测试缺口（REFACTOR / TEST）

- [x] **REFT-01**: cross-flavor permission contract 测试矩阵（Milestone 1 删完后只剩 Cursor 单 flavor，矩阵简化为「Cursor permission mode → CLI flag」的完整覆盖；新增 mode 加入需补充矩阵行）
- [x] **REFT-02**: SSE reconnect / patch-loss 不变量测试。覆盖断线重连后前端最终收敛到正确状态
- [x] **REFT-03**: Auth 路由 negative cases 测试。bad token、过期 JWT、replayed JWT、空 body
...
- [x] **CUT-12**: 文档与营销内容收敛。更新 root `README.md` / `cli/README.md` / `hub/README.md` / `web/README.md` / `AGENTS.md` 到 Cursor-only 单 agent 描述；删除 `website/` 营销站；删除 `docs/` 中提及 claude/codex/gemini/opencode/telegram/voice/serverchan 的页面

### Milestone 1 验收（VERIFY）

- [x] **VRFY-01**: `bun typecheck` 全绿；`bun run test` 全绿；lint 全绿
- [x] **VRFY-02**: 0 循环依赖（`madge` 或同类工具验证 `cli/` / `hub/` / `web/` 三个包内 + 跨包）
- [x] **VRFY-03**: 全仓库 ripgrep 不出现 `claude` / `codex` / `gemini` / `opencode` / `telegram` / `serverchan` / `elevenlabs` / `tunwg` / `namespace` 残留（白名单：`.planning/codebase/` 历史快照、`CHANGELOG.md` 历史条目、git 历史中的 commit message）
- [x] **VRFY-04**: Cursor + Tailscale 场景手动验收。本机启动 `hapi runner` 与 hub；从 Tailscale 内网手机端（Web PWA）创建 Cursor session → 一轮交互 → 杀掉 hub 重启 → 状态恢复 → 继续交互——全流程通过
...
| REFH-02 | Phase 8 (Hub internal decoupling) | Complete |
| REFH-03 | Phase 8 (Hub internal decoupling) | Complete |
| REFH-04 | Phase 8 (Hub internal decoupling) | Complete |
| REFW-01 | Phase 9 (Web internal decoupling) | Complete |
| REFW-02 | Phase 9 (Web internal decoupling) | Complete |
| REFW-03 | Phase 9 (Web internal decoupling) | Complete |
| REFC-01 | Phase 10 (Config cleanup) | Complete |
| REFC-02 | Phase 10 (Config cleanup) | Complete |
| REFT-01 | Phase 11 (Test gap fill) | Complete |
| REFT-02 | Phase 11 (Test gap fill) | Complete |
| REFT-03 | Phase 11 (Test gap fill) | Complete |
| CUT-12 | Phase 12 (Docs cleanup & milestone verification) | Complete |
| VRFY-01 | Phase 12 (Docs cleanup & milestone verification) | Complete |
| VRFY-02 | Phase 12 (Docs cleanup & milestone verification) | Complete |
| VRFY-03 | Phase 12 (Docs cleanup & milestone verification) | Complete |
| VRFY-04 | Phase 12 (Docs cleanup & milestone verification) | Complete |
